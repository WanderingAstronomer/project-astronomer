...
| **Capability inventory** | living | What the collaborator can read, run, and reach — and, in the load-bearing section, where it is systematically wrong. Every other instrument in the framework declares its own error; this applies the rule to the one doing most of the observing. Gated on a condition: required whenever something other than the operator is doing the observing. | **Lite, conditional** | [`capability-inventory.template.md`](capability-inventory.template.md) |
...
| Capability inventory | Before the first observation window, as part of `starting-a-project` | Whenever the environment changes — and re-dated every time | Never | When the collaborator stops working on the project |
