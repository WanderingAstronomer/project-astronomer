...
| **template carries** | Rules that bite at **session** time appear in `install/CLAUDE.md.template`, not only in `install/README.md` — and the README still explains anything the template carries | The `AST-D-` namespacing rule, absent from the always-loaded file since the framework had one. A **second** consuming project's session wrote a bare `D-044` one paragraph after citing `AST-D-049`, which is the third instance of the namespace class |
